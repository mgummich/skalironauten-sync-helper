...
## Overview
Daily-standup companion for a small German dev team. One job, under a minute at 09:00: read today's **Aktionstag** (commemorative day from a 930-entry list), do the **Mood Check** (random meme scale image + 1–9), download or copy the image into the Teams chat. Local-only (localStorage), no accounts, no backend, no team view.

Priority: **Aktionstag and Mood Check are the whole page.** Standup-Notizen (Gestern/Heute/Blocker + text copy) were **removed** in the latest revision — do not build them.
...
Desktop under hero: Mood card in a single centered column `max-w-[560px]`. Content width 1120 centered, `py-8`.
...
- **Action rows (new):** directly under the image. Row 1: left status `text-sm text-slate-600 min-w-0` ("Zufallsvorschlag · noch ungenutzt" / "· 1× genutzt (24.08.)" / loading: "Zufallsvorschlag wird geladen …"), right secondary pill 40px `border-[1.5px] border-red-700 text-red-700 rounded-full px-3.5 text-sm font-semibold whitespace-nowrap shrink-0` with lucide `Dices` 18px, "**Zufällig wählen**" → draws a new random image (prefers unused, avoids last 5, replaces the current image with a 150ms crossfade; clears any selected number? **No** — selection stays). Row 2: two tertiary pills 40px `flex-1 border border-slate-300 text-slate-700 rounded-full text-sm font-semibold` — `Download` "**Herunterladen**" (saves the original file, filename `mood-{id}.{ext}`) and `Copy` "**Bild kopieren**" (writes the image as PNG blob to the clipboard via `navigator.clipboard.write([new ClipboardItem({"image/png": blob})])`, convert non-PNG via canvas; success state 2s: `Check` icon + "Kopiert" for the pressed pill; fallback if unsupported: open download). Both disabled (`text-slate-400`) while loading. Desktop (2c): status line on top, the three pills in one wrapping row below (`flex-wrap gap-2`), order Herunterladen · Bild kopieren · Zufällig wählen.
...
- **Mood Check integration:** the status row shows usage of the suggested image; "Zufällig wählen" re-rolls (see §2). Picking a specific image happens from the Moods tab ("Für heute wählen"). Random suggestion prefers unused images.

## Interactions & Behavior
- **Image to Teams:** "Bild kopieren" → paste into Teams chat. Clipboard image write needs a user gesture and a secure context; Safari requires `ClipboardItem` with a Promise<Blob>. If `navigator.clipboard.write` is unavailable, trigger the download instead and show "Heruntergeladen".
- Aktionstag stepper wraps not; prev disabled at 1, next disabled at n. Change = 150ms crossfade.
- Mood: image chosen randomly per dialog open (avoid repeating last 5); "Zufällig wählen" re-rolls with the same rule; saved value `{date, value, imageId, time}`.
- Copy/download success: 200ms color transition, hold 2s, 200ms back. `aria-live=polite` on status text.
...
- Removed: `notes:*` keys (delete on migration).
